...
def filter_data(df, mass_cutoff, time_diff_cutoff, intensity_cutoff):
    df["Sum Intensity"] = df["Sum Intensity"].astype(float) 
    filtered_df = df[(df["Monoisotopic Mass"] >= mass_cutoff) & 
                     ((df["Stop Time (min)"] - df["Start Time (min)"]) <= time_diff_cutoff) & 
                     ( np.log10(df["Sum Intensity"]) >= intensity_cutoff )]
...
def run_process(df, mass_cutoff, time_diff_cutoff, mass_table, similarity_cutoff, intensity_cutoff):
    filtered_df = filter_data(df, mass_cutoff, time_diff_cutoff, intensity_cutoff)
...
def main():
    ...
    if df is not None:
        ...
        intensity_cutoff = st.sidebar.number_input("Sum Intensity log10(5000)=3.7", min_value=3.0, value=3.7 )
        ...
        if st.sidebar.button("Four nucleotides + D & generate network"):
#            with st.spinner('Wait for it...'):
            run_process(df, mass_cutoff, time_diff_cutoff, mass_table, similarity_cutoff, intensity_cutoff)
