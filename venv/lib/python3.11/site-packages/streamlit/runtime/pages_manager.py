...
class PagesStrategyV1:
    """
    Strategy for MPA v1. This strategy handles views being set directly
    by a call to `st.navigation`. The key differences here are:
    - The views are defined by the existence of a `views` directory
    ...
    NOTE: Thread safety of the views is handled by the source_util module
    """

    is_watching_pages_dir: bool = False
    pages_watcher_lock = threading.Lock()

    # This is a static method because we only want to watch the views directory
    ...
    def watch_pages_dir(pages_manager: PagesManager):
        with PagesStrategyV1.pages_watcher_lock:
            ...
            pages_dir = pages_manager.main_script_parent / "views"
            watch_dir(
                str(pages_dir),
    ...
    def get_initial_active_script(
        ...
        if page_script_hash:
            ...
            # the first script run request before the list of views has been
            ...
            return next(
                filter(
                    ...
                    # types of views), so we add `p and` at the beginning of
        ...
        # Safe because views will at least contain the app's main page.
    ...
    def set_pages(self, _pages: dict[PageHash, PageInfo]) -> None:
        raise NotImplementedError("Unable to set views in this V1 strategy")
...
class PagesStrategyV2:
    """
    Strategy for MPA v2. This strategy handles views being set directly
    by a call to `st.navigation`. The key differences here are:
    - The views are set directly by the user
    ...
    - We rely on views manager to retrieve the intended page script per run

    NOTE: We don't provide any locks on the views since the views are not
    shared across sessions. Only the user script thread can write to
    views and the event loop thread only reads
    """
    ...
    def get_pages(self) -> dict[PageHash, PageInfo]:
        # If views are not set, provide the common page info where
    ...
    def register_pages_changed_callback(
        ...
        # V2 strategy does not handle any views changed event
        return lambda: None


class PagesManager:
    """
    PagesManager is responsible for managing the set of views based on the
    strategy. By default, PagesManager uses V1 which relies on the original
    assumption that there exists a `views` directory with all the scripts.

    If the `views` are being set directly, the strategy is switched to V2.
    This indicates someone has written an `st.navigation` call in their app
    which informs us of the views.

    NOTE: Each strategy handles its own thread safety when accessing the views
    """
    ...
    def set_pages(self, pages: dict[PageHash, PageInfo]) -> None:
        # Manually setting the views indicates we are using MPA v2.
        if isinstance(self.pages_strategy, PagesStrategyV1):
            if os.path.exists(self.main_script_parent / "views"):
                _LOGGER.warning(
                    "st.navigation was called in an app with a views/ directory. "
    ...
    def get_page_script(self, fallback_page_hash: PageHash = "") -> PageInfo | None:
        # We assume the views strategy is V2 cause this is used
    ...
    def register_pages_changed_callback(
        ...
        """Register a callback to be called when the set of views changes.
