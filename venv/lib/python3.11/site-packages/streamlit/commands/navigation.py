...
def navigation(
    ...
    *,
    ...
    """
    Configure the available views in a multipage app.

    Call ``st.navigation`` in your entrypoint file to define the available
    views for your app. ``st.navigation`` returns the current page, which can
    ...
    each of your views. Streamlit executes the entrypoint file with every app
    rerun. To execute the current page, you must call the ``.run()`` method on
    the ``StreamlitPage`` object returned by ``st.navigation``.

    The set of available views can be updated with each rerun for dynamic
    navigation. By default, ``st.navigation`` displays the available views in
    ...
    your app will ignore the ``views/`` directory (across all sessions).

    Parameters
    ----------
    pages : list[page-like], dict[str, list[page-like]]
        The available views for the app.

        To create a navigation menu with no sections or page groupings,
        ``views`` must be a list of page-like objects. Page-like objects are
        ...
        menu, ``views`` must be a dictionary. Each key is the label of a
    ...
    position : "sidebar" or "hidden"
        ...
        If there is only one page in ``views``, the navigation will be hidden
    ...
    expanded : bool
        ...
        button to view more options when there are too many views to display.
    ...
    manages your app's navigation and serves as a router between views.

    **Example 1: Use a callable or Python file as a page**

    You can declare views from callables or file paths. If you pass callables
    ...
    **Example 2: Group views into sections and customize them with ``st.Page``**
    ...
    views are in the same directory. However, you can use Python files from
    ...
    **Example 3: Stateful widgets across multiple views**

    Call widget functions in your entrypoint file when you want a widget to be
    stateful across views. Assign keys to your common widgets and access their
    values through Session State within your views.
    ...
    >>>
    >>> # Widgets shared by all the views
    ...
    # Inform our page manager about the set of views we have
