...
def switch_page(page: str | Path | StreamlitPage) -> NoReturn:  # type: ignore[misc]
    ...
    architecture (your main Python file or a Python file in a ``views/``
    ...
    >>> if st.button("Page 1"):
    >>>     st.switch_page("views/page_1.py")
    >>> if st.button("Page 2"):
    >>>     st.switch_page("views/page_2.py")
    ...
    if isinstance(page, StreamlitPage):
        ...
        if len(matched_pages) == 0:
            raise StreamlitAPIException(
                f"Could not find page: `{page}`. Must be the file path relative to the main script, from the directory: `{os.path.basename(main_script_directory)}`. Only the main app file and files in the `views/` directory are supported."
            )

        page_script_hash = matched_pages[0]["page_script_hash"]

    # We want to reset query params (with exception of embed) when switching views
