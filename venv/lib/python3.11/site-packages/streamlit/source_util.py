...
_on_pages_changed = Signal(doc="Emitted when the views directory is changed")
...
def get_pages(main_script_path_str: ScriptPath) -> dict[PageHash, PageInfo]:
    ...
    # Avoid taking the lock if the views cache hasn't been invalidated.
    ...
    with _pages_cache_lock:
        ...
        #       easier for us when we need to iterate over views.
        ...
        pages_dir = main_script_path.parent / "views"
        page_scripts = sorted(
            [
                f
...
def register_pages_changed_callback(
    ...
    # weak=False so that we have control of when the views changed
